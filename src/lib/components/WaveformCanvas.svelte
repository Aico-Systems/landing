    ...
    const animate = (currentTime: number) => {
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        if (!gradient) {
          ...
          // Brand colors: mint → petrol
          gradient.addColorStop(0, `rgba(167, 243, 208, ${alpha1})`);
          gradient.addColorStop(1, `rgba(20, 91, 122, ${alpha2})`);

          if (gradientCache.size > 100) gradientCache.clear();
...
<style>
  ...
  :global([data-theme="light"]) .consent-text {
    ...
    border-top-color: rgba(20, 91, 122, 0.15);
  }

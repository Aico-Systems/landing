<script lang="ts">
  ...
  const baseCities: BaseCity[] = [
    ...
    // South America (better spread)
    {
      ...
      altitude: 0.055,
      intensity: 0.72,
      region: "Brazil · Business Capital",
    },
    {
      id: "buenos-aires",
      name: "Buenos Aires",
    ...
    },
    {
      ...
      altitude: 0.048,
      intensity: 0.61,
      region: "South Africa · Tech Capital",
    },
  ...
  function createFlatTexture(themeKey: ThemeKey): string {
    ...
    palette.gradient.forEach(({ stop, color }) =>
    ...
    const highlight = ctx.createRadialGradient(
      canvas.width * cx,
      canvas.height * cy,
      canvas.height * palette.highlight.innerRadius,
      canvas.width * cx,
      canvas.height * cy,
      canvas.height * palette.highlight.outerRadius,
    );
    ...
    const vignette = ctx.createRadialGradient(
      canvas.width / 2,
      canvas.height / 2,
      canvas.height * palette.vignette.start,
      canvas.width / 2,
      canvas.height / 2,
      canvas.height * palette.vignette.end,
    );
  ...
  function getFlatTexture(themeKey: ThemeKey): string {
    if (!flatTextureCache[themeKey])
      flatTextureCache[themeKey] = createFlatTexture(themeKey);
  ...
  const sectionViews: Record<string, SectionView> = {
    ...
    "how-it-works": {
      pov: { lat: 6, lng: 46, altitude: 2.15 },
      offset: [-40, -20],
    },
    features: {
      pov: { lat: -14, lng: -48, altitude: 1.85 },
      offset: [120, 50],
    },
    "use-cases": {
      pov: { lat: 28, lng: 94, altitude: 1.9 },
      offset: [-200, 30],
    },
    testimonials: {
      pov: { lat: -24, lng: -104, altitude: 2.05 },
      offset: [150, -60],
    },
  ...
  function applyTheme(nextTheme: ThemeKey) {
    ...
    if (!cachedLights[nextTheme]) {
      const ambientLight = new THREE.AmbientLight(
        config.lights.ambient.color as any,
        config.lights.ambient.intensity,
      );
      const keyLight = new THREE.DirectionalLight(
        config.lights.key.color as any,
        config.lights.key.intensity,
      );
      keyLight.position.set(
        config.lights.key.position[0],
        config.lights.key.position[1],
        config.lights.key.position[2],
      );
      const fillLight = new THREE.DirectionalLight(
        config.lights.fill.color as any,
        config.lights.fill.intensity,
      );
      fillLight.position.set(
        config.lights.fill.position[0],
        config.lights.fill.position[1],
        config.lights.fill.position[2],
      );
      const globeLights: THREE.Light[] = [ambientLight, keyLight, fillLight];

      if ("rim" in config.lights) {
        const rimLight = new THREE.DirectionalLight(
          config.lights.rim.color as any,
          config.lights.rim.intensity,
        );
        rimLight.position.set(
          config.lights.rim.position[0],
          config.lights.rim.position[1],
          config.lights.rim.position[2],
        );
  ...
  onMount(() => {
    ...
    globe
      ...
      .arcStroke("stroke")
      .arcDashLength(0.4) // "Airplane" length
      .arcDashGap(2) // Gap between planes
      .arcDashInitialGap((d: any) => Math.random() * 2) // Random start positions
      .arcDashAnimateTime((d: any) => d.dashTime) // Use calculated speed
      .arcCurveResolution(quality.arcCurveResolution)
      .arcCircularResolution(quality.arcCircularResolution)
      .ringsData([]);

    // Initial camera position + offset for hero
    const hero = sectionViews.hero;
    globe.pointOfView(
      { lat: hero.pov.lat, lng: hero.pov.lng, altitude: hero.pov.altitude },
      0,
    );

<script lang="ts">
  ...
  import { prefersReducedMotion, getConnectionQuality, rafScheduler } from './utils/performance';
  ...
  async function initiateCall() {
    ...
    try {
      ...
      const apiUrl = window.location.hostname === 'landing.aicoflow.xyz' 
...
<div class="waveform-wrapper">
  ...
  <div class="phone-input-overlay" class:expanded={inputFocused}>
...
<style>
  ...
  .phone-input-overlay.expanded .input-panel {
    gap: 16px;
  }
  ...
  :global([data-theme="light"]) .phone-input:focus {
  ...
  .phone-input-overlay.expanded .consent-text {
    max-height: 50px;
    opacity: 1;
    margin-top: 10px;
  }
  ...
  :global([data-theme="light"]) .status-message.success {
  ...
  :global([data-theme="light"]) .status-message.error {
